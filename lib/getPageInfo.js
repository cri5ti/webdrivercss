    ...
    async.waterfall([
        ...
        function(res, cb) {
            ...
            if (typeof element == 'number' || parseInt(element) == element) {
                // element is a ELEMENT ID
                promise = that.instance.execute(script, { ELEMENT: element } )
                    .then(function(res) { return res.value; });
            } else if (typeof element.ELEMENT == 'number') {
                // element is a JSON ELEMENT
                promise = that.instance.execute(script, element)
                    .then(function(res) { return res.value; });
            } else {
                // element is a selector
                promise = that.instance.selectorExecute(element, script);
            }

            promise.then(function (elementInfo) {
